...
$.extend(Microfiche.prototype, {
  ...
  // We create our film element, which weвЂ™ll slide back and forth in the screen.
  // Before appending any extra elements, we detach the existing children,
  // append them to film, and tell them to float so theyвЂ™ll (hopefully) lay-out
  ...
  prepareCyclic: function() {
    ...
    cloneL.find(this.options.dontClone).remove();
    cloneR.find(this.options.dontClone).remove();

    cloneL.prependTo(this.film).css({ position: 'absolute', left: -w + 'px' });
    cloneR.appendTo(this.film).css({ position: 'absolute', left: w + 'px' });
  },

  // This slightly strange process tries to ensure we donвЂ™t get any wrapping
  ...
  //
  // If weвЂ™re not already in drag-mode, we check to see if the horizontal
  ...
  // to be a drag, weвЂ™ll deduce the new target value for x, ensure Microfiche
  ...
  // Handle what happens in cyclic mode if weвЂ™ve slipped off at either end.
  ...
  // Slide to the previous screenвЂ™s-worth of slides.
  prev: function() {
    this.slideByPages(-1);
  },

  // Slide to the next screenвЂ™s-worth of slides.
  next: function() {
    this.slideByPages(1);
  },
